...
public class VersionByCommitFixture : Lg2sHelperBase
{
    ...
    */
    ...
    static string VersionFor(Repository repo, string branchName)
    {
        ...
        var rd = ReleaseDateFinder.Execute(repo, vab);
        var v = vab.ToKeyValue();

        return string.Format("{0} -> ReleaseDate OriginalDate ({1} on {2}) / Date ({3} on {4})",
            v["SemVer"], rd.OriginalCommitSha.Substring(0, 7), rd.OriginalDate.ToString("o"), rd.CommitSha.Substring(0, 7), rd.Date.ToString("o"));
    }
